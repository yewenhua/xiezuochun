...
import so.voc.vhome.adapter.DeviceShareRecordAdapter;
 ...
 * @Date：2019/6/5 14:14
 */
public class DeviceShareRecordActivity extends BaseActivity<ActivityShareRecordBinding> implements OnRefreshListener, OnLoadMoreListener {
    public static final String TAG = DeviceShareRecordActivity.class.getSimpleName();

    private DeviceShareRecordAdapter mAdapter;
    private List<DeviceShareRecordModel.ContentBean> contentBeans;
    ...
    protected void initView() {
        setTitle(CommonUtils.getString(R.string.share_record));
        contentBeans = new ArrayList<>();
        mAdapter = new DeviceShareRecordAdapter(this);
     ...
     * @param contentBean
     */
    private void delShareRecord(DeviceShareRecordModel.ContentBean contentBean) {
        OkGo.<ResponseModel>post(VHomeApi.DEVICE_SHARE_DELETE + contentBean.getId())
    ...
    private void getShareRecord() {
        OkGo.<ResponseModel<DeviceShareRecordModel>>get(VHomeApi.DEVICE_SHARE_SEARCH)
                .params(Constants.ACCESS_TOKEN, String.valueOf(SharedPreferencesUtils.get(Constants.ACCESS_TOKEN, "")))
                .params("pageNumber", pageNumber)
                .params("pageSize", "20")//不给默认20
                .params("deviceShareListType", "DEVICE_OWNER")
                .tag(this)
                .execute(new StringCallback<ResponseModel<DeviceShareRecordModel>>() {
                    @Override
                    public void onSuccess(Response<ResponseModel<DeviceShareRecordModel>> response) {
                        super.onSuccess(response);
                        bindingView.srlShareRecord.finishRefresh();
                        bindingView.srlShareRecord.finishLoadMore();
                        if (response.body().code == 0) {
                            DeviceShareRecordModel deviceShareRecordModel = response.body().data;
                            List<DeviceShareRecordModel.ContentBean> contentBeanList = deviceShareRecordModel.getContent();
                            if (pageNumber == 1){
                                ...
                                for (DeviceShareRecordModel.ContentBean contentBean: contentBeanList) {
